...
#if defined(NDEBUG)
#error "EcoPay cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uecp) are considered zero fee (for relaying and mining)
...
{
    int64_t nSubsidy = 0;
    int64_t nSubsidyHalvingInterval = 100000;
    ...
    if (nHeight < Params().LAST_POW_BLOCK())
        ...
        for (int i = nSubsidyHalvingInterval; i <= nHeight - 600000; i += nSubsidyHalvingInterval) {
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // ECP: We use certain sporks during IBD, so check to see if they are
